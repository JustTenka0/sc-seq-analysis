...
2. **Normalization & Feature Selection:** Log-normalization and identification of top highly variable genes (HVGs).
3. **Dimensionality Reduction:** Linear (PCA) and non-linear (UMAP/t-SNE) dimensionality reduction.
4. **Clustering & Annotation:** Unsupervised graph-based clustering (Louvain algorithm) and cell-type annotation using canonical cell markers (e.g., T cells, B cells, Monocytes, NK cells).
5. **Differential Expression Analysis:** Identification of cluster marker genes and pathway enrichment analysis.
